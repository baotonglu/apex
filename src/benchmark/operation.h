...
void concurr_insert(Tree<T, P> *index, struct Range *_range) {
  ...
  while (i < round) {
    ...
    for (uint64_t j = i * EPOCH_DURATION; j < end; ++j) {
      bool ret =
          index->insert(key_array[j], reinterpret_cast<P>(&key_array[j]));
      if (!ret) fail_insert++;
    }
    ++i;
  }

  {
    ...
    for (i = EPOCH_DURATION * round; i < num; ++i) {
      bool ret =
          index->insert(key_array[i], reinterpret_cast<P>(&key_array[i]));
      if (!ret) fail_insert++;
    }
  }

  auto workload_end_time = std::chrono::high_resolution_clock::now();
  _range->total_time = std::chrono::duration_cast<std::chrono::nanoseconds>(
...
void concurr_search(Tree<T, P> *index, struct Range *_range) {
  ...
  _range->total_time = std::chrono::duration_cast<std::chrono::nanoseconds>(
                           workload_end_time - workload_start_time)
                           .count();
  std::cout << "Not found during search = " << num - found << " with total "
            << total_num << std::endl;
  end_notify();
}
...
void range_debug(Tree<T, P> *index, struct Range *_range) {
  set_affinity(_range->id);
  uint64_t num = _range->num;
  T *key_array = reinterpret_cast<T *>(
      _range->workload);  // This array is also the sorted keys that is indexes
                          // by the index
  uint64_t found = 0;
  typedef std::pair<T, P> V;
  V *result = new V[TO_SCAN];
  ...
  for (uint64_t i = 0; i < num; ++i) {
    ...
    for (int j = 0; j < found; ++j) {
      if (result[j].first != key_array[pos + j]) {
        ...
        std::cout << "--------------------start list all keys in result "
                     "array----------------------"
                  << std::endl;
        for (int k = 0; k < found; ++k) {
          std::cout << "Result " << k << " = " << result[k].first << std::endl;
          std::cout << "Coorect result " << k << " = " << key_array[pos + k]
                    << std::endl;
        }
        exit(-1);
      }
