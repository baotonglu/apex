...
// Moreover, need to use static member to make all allocatoion in a single
// memory pool
 ...
 public:
  ...
  static bool Initialize(const char* pool_name, size_t pool_size) {
    ...
    if (!FileExists(pool_name)) {
      ...
      std::cout << "pool opened at: " << std::hex << pm_pool_ << std::dec
                << std::endl;
    } else {
  ...
  static void IncreaseAllocatorNum() { allocator_num++; }

  static void DecreaseAllocatorNum() { allocator_num--; }
  ...
  static void AlignAllocate(void** ptr, size_t size) {
    PMEMoid tmp_ptr;
    auto ret = pmemobj_alloc(pm_pool_, &tmp_ptr, size + 64, TOID_TYPE_NUM(char),
                             NULL, NULL);
    if (ret) {
  ...
  static void AlignZAllocate(void** ptr, size_t size) {
    PMEMoid tmp_ptr;
    auto ret =
        pmemobj_zalloc(pm_pool_, &tmp_ptr, size + 64, TOID_TYPE_NUM(char));
    if (ret) {
      std::cout << "Fail logging: " << ret << "; Size = " << size << std::endl;
      LOG_FATAL("Allocate: Allocation Error in PMEMoid 1");
    }
  ...
  static void Allocate(void** ptr, size_t size) {
    PMEMoid tmp_ptr;
    auto ret = pmemobj_alloc(pm_pool_, &tmp_ptr, size, TOID_TYPE_NUM(char),
                             NULL, NULL);
    if (ret) {
      std::cout << "Fail logging: " << ret << "; Size = " << size << std::endl;
      LOG_FATAL("Allocate: Allocation Error in PMEMoid 1");
  ...
  static void Allocate(PMEMoid* ptr, size_t size) {
    auto ret =
        pmemobj_alloc(pm_pool_, ptr, size, TOID_TYPE_NUM(char), NULL, NULL);
    if (ret) {
      std::cout << "Fail logging: " << ret << "; Size = " << size << std::endl;
      LOG_FATAL("Allocate: Allocation Error in PMEMoid 1");
    }
  ...
  static void Allocate(PMEMoid* pm_ptr, uint32_t alignment, size_t size,
                       int (*alloc_constr)(PMEMobjpool* pool, void* ptr,
                                           void* arg),
                       void* arg) {
    auto ret = pmemobj_alloc(pm_pool_, pm_ptr, size, TOID_TYPE_NUM(char),
                             alloc_constr, arg);
    if (ret) {
...
class allocator : BasePMPool {
 public:
  ...
  pointer allocate(size_type n) {
    ...
    PMEMoid tmp_ptr;
    auto ret = pmemobj_alloc(pm_pool_, &tmp_ptr, (size_t)(n * sizeof(T)),
                             TOID_TYPE_NUM(char), NULL, NULL);
    if (ret) {
      std::cout << "Fail logging: " << ret << "; Size = " << n * sizeof(T)
                << std::endl;
