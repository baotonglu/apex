 ...
 public:
  ...
  static void New(PMEMoid* new_node, size_t num_children = 0) {
    my_alloc::BasePMPool::Allocate(
        new_node,
        sizeof(AlexModelNode<T, P>) + num_children * sizeof(AlexNode<T, P>*));
    auto new_model_node =
...
class AlexDataNode : public AlexNode<T, P> {
 public:
...
#else
  // My new cost model
  double num_insert_cost_ = 0;  // including insert in nornaml array, in
                                // overflow list, create overflow list
  ...
  class OrderIterator {
   public:
    ...
    uint64_t cur_bitmap_data_ =
        0;  // caches the relevant data in the current bitmap position
    V* sorted_array_ = nullptr;  // the value is attached with the key, so I
  ...
  double compute_overflow_frac(
      const V* values, int num_keys, int data_capacity,
  ...
  void initialize(int num_keys, double density, double stash_frac = 0.05) {
    ...
    real_data_capacity_ = data_capacity_ + PROBING_LENGTH -
                          1;  // real data capacity in main array
  ...
  void bulk_load(const V values[], int num_keys,
    ...
    if ((data_capacity_ % scale_factor_) != 0) {
      scale_parameters_[initial_parameter_size].expansion_threshold_ = std::min(
          std::max(
              static_cast<int>((total_capacity % SCALE_FACTOR) * kMaxDensity_),
              num_keys + 1),
          total_capacity);
    }
  ...
  void bulk_load_from_existing(
      ...
      int precomputed_num_actual_keys = -1, bool rebuild_sorted_node = false,
  ...
  inline int range_scan_by_size(const T& key, uint32_t to_scan, V* result,
    ...
    int remaining_scan =
        to_scan;  // Number of remaining kv records for scanning
    ...
    *ret_offset = 0;
    ...
    if (scale_parameters_[parameter_pos].overflow_stash_count_ == 0) {
      overflow_stash_type::New(&P_data_slots_, key, payload, invalid_key_,
                               parameter_pos);
      new_overflow_stash =
          reinterpret_cast<overflow_stash_type*>(pmemobj_direct(P_data_slots_));
      new_overflow_stash->link_ = first_block_;
      clwb(&(new_overflow_stash->link_));
      sfence();
      first_block_ = new_overflow_stash;
  ...
  inline void link_overflow_stash_with_sync(
      int parameter_pos, overflow_stash_type* new_overflow_stash) {
  ...
  static void New_from_existing(PMEMoid* ptr, self_type* old_node) {
    ...
    my_alloc::BasePMPool::Allocate(ptr, kCacheLineSize, sizeof(self_type),
                                   callback, reinterpret_cast<void*>(old_node));
  }
