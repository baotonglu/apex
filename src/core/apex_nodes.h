...
#include "../util/new-preallocpool.h"
#include <tuple>
#include <utility>
#include <algorithm>
#include <atomic>
...
class AlexModelNode : public AlexNode<T, P> {
 public:
...
#ifdef DRAM_MODEL  
  static void New(void** new_node, size_t num_children = 0){
    align_zalloc(new_node, sizeof(AlexModelNode<T, P>) + num_children * sizeof(AlexNode<T, P>*));
    auto new_model_node = reinterpret_cast<AlexModelNode<T, P>*>(*new_node);
  ...
  inline bool get_read_lock(){
    ...
    uint32_t old_value = v & lockMask;
    auto new_value = ((v & lockMask) + 1) & lockMask; // rounded problem? no, since there are at most 24 threads, lol
    while (!CAS(&lock_, &old_value, new_value)){
      if((old_value & lockSet) || this->is_obsolete_){
        return false;
      }
  ...
  inline void reset_rw_lock(){ 
    lock_ = 0;
    //my_alloc::BasePMPool::Persist(&lock_, sizeof(lock_));
    clwb(&lock_);
    sfence();
  }
...
// Variables for determining append-mostly behavior
  ...
  ~AlexDataNode() {
    ...
    value_allocator().deallocate(data_slots_, real_data_capacity_);
#endif
#endif 
...
#endif
{
  ...
  // BT: return a allocator instance, to do the allocation 
  ...
  T first_key() const {
#ifdef HASH
//std::cout << "Get the first key" << std::endl;
  ...
  T last_key() const {
#ifdef HASH
//std::cout << "Get the last key" << std::endl;
    order_iterator_type it(this);
    if(!it.is_end()){
      return it.last_key();
    }
    ...
    *ret_offset = -1;
    for(int i = 0; i < 5; ++i){
      ...
      if(bitmap & bitmapSet){
         ...
         *ret_offset = i; // The slot which could used to add overflow_stash
        break;
      }
    }
...
*/
  ...
  static void build_node_implicit_from_existing(self_type* node, int left,
      ...
      int last_overflow_position = -1;
      int num_in_last_overflow_blocks = 0;
//std::cout << "build node implicit from exisitng" << std::endl;
      order_iterator_type it(node, left, true);
  ...
  void initialize(int num_keys, double density, double stash_frac = 0.05) {
    ...
    stash_capacity_ = std::max(static_cast<int>(data_capacity_ * stash_frac), PROBING_LENGTH);
    if(data_capacity_ >= (2 * PROBING_LENGTH)) data_capacity_ -= stash_capacity_;
...
#ifdef COLLECT_ALEX_WRITE
      smo_num_write += sizeof(*this);
      smo_num_write += sizeof(V) * real_data_capacity_ / 256;
#endif
      if (num_actual_keys == 0) {
        expansion_threshold_ = data_capacity_;
        scale_parameters_[0].expansion_threshold_ = data_capacity_;
...
#ifdef COLLECT_ALEX_WRITE
      smo_link_block_alloc += linked_block_num(meta_info_array_, real_data_capacity_ / TABLE_FACTOR + 1);
#endif
