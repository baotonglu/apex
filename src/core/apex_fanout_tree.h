...
int find_best_fanout_existing_node(const AlexModelNode<T, P, Alloc>* parent,
 ...
 // double right_boundary_value = node->max_limit_;
  ...
  // debug... display the value computed using new model and old model
  if(((right_boundary_value - debug_max_vlaue) < precise_value) && ((right_boundary_value - debug_max_vlaue) > (-precise_value)))
  {
    printf("the parent computed mid value is %.25f\n", (start_bucketID + repeats / 2 - parent->model_.b_) / parent->model_.a_);  
    printf("the model computed mid value is %.25f\n", (1 - base_model.b_ * 2) / (base_model.a_ * 2));  
  }
