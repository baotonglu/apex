...
struct FTNode {
  ...
  int num_keys = 0;
  void* data_node = nullptr;  // store the pointer the data node generated by
                              // the corresponding fanout tree node
};
...
double compute_level(const std::pair<T, P> values[], int num_keys,
  ...
  for (int i = 0; i < fanout; i++) {
    ...
    // boundary_value = (static_cast<double>(i + 1) / fanout - node->model_.b_)
    // / node->model_.a_; if(!std::is_floating_point<T>::value){
...
std::pair<int, double> find_best_fanout_bottom_up(
  ...
  for (int fanout = 2, fanout_tree_level = 1; fanout <= max_fanout;
    ...
    // I cannot use && since maybe in some bottom layer, it is really hard to
    // fit => always has larger than 100 cost For the bottom layer, we really
    // need a way to solve non-uniform distribution problem
    if ((static_cast<double>(num_keys) / total_keys < 0.1 ||
         best_cost <= 100) &&
        fanout_costs.size() >= 6 &&
        fanout_costs[fanout_costs.size() - 1] >
            fanout_costs[fanout_costs.size() - 2] &&
      ...
      if (node_split_cost < tree_node.cost) {
        ...
        for (int i = 0; i < 2; i++) {
          ...
          new_level.push_back({fanout_tree_level, 2 * tree_node.node_id + i,
                               ...
                               0, 0, true, node_stats[i].num_search_cost,
...
int find_best_fanout_existing_node(const AlexModelNode<T, P, Alloc>* parent,
  ...
  double left_boundary_value = (start_bucketID - parent->model_.b_) /
                               parent->model_.a_;  // left key value
  double right_boundary_value = (end_bucketID - parent->model_.b_) /
                                parent->model_.a_;  // right key value
  ...
  if (right_boundary_value != node->max_limit_) {
    ...
    printf("the min limit is %.25f\n", node->max_limit_);
    double parent_right_limit =
        (parent->num_children_ - parent->model_.b_) / parent->model_.a_;
  ...
  for (int fanout = 1, fanout_tree_level = 0; fanout <= max_fanout;
    ...
    for (int i = 0; i < fanout; i++) {
      ...
      LinearModel<T> model;
      typename AlexDataNode<T, P>::order_iterator_type it(node, left_boundary,
                                                          true);
      ...
      new_level.push_back({fanout_tree_level, i, node_cost, left_boundary,
                           ...
                           stats.num_search_cost, stats.num_insert_cost,
