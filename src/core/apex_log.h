...
// LOG descriptor, specific to APEX, the idea is somewhat similar to micro-log
// used in FPTree

// Redo-Undo log, could be used in data node resize and overflow stash
// allocation After new_node_ is set, we need redo the step Need cacheline
// alignment
struct __attribute__((aligned(64))) ResizeLog {
  ...
  int progress_ =
      0;  // 0 means nothing ocurrs; 1 means resize is in initial phase (undo);
          // 2 means the data node is well prepared, redo is better
...
struct RootExpandLog {
  void clear_log() {
    ...
    my_alloc::BasePMPool::Persist(&root_expand_in_progress_,
                                  sizeof(root_expand_in_progress_));
  ...
  // -1 means exapnd left start; 1 means expand right end; 0 means no root
  // exapnd; -2 means creat new root node left; 2 means create new root node
  // right
...
// SMO for node split
// Redo after the node has allocated the new data nodes since the node links are
// already updated after this
struct __attribute__((aligned(64))) SplitSidewayDownwardsLog {
  ...
  PMEMoid child_nodes_ = OID_NULL;  // For > 2 children; should also have the
                                    // storage for tree level (also local depth)
...
class MyLog {
 public:
  ...
  OverflowDesc
      overflow_desc_;  // The descriptor for safe allocation of overflow block
  uint64_t
      in_use;  // 0 means that this node is not in-use; registe using thread ID
  uint64_t dummy[3];
};
