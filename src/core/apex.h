...
class Alex : public Tree<T, P> {
 ...
 public:
  ...
  explicit Alex(InputIterator first, InputIterator last,
      ...
      : allocator_(alloc) {
    set_default_parameters();
    std::cout << "Get Num = " << my_alloc::BasePMPool::allocator_num << std::endl;
    std::vector<V> values;
    for (auto it = first; it != last; ++it) {
      values.push_back(*it);
    }
...
// BT: add concurrency now, if the concurrency race condition occurs, abort and return nullptr
  ...
  data_node_type* get_leaf(
    ...
    while (true) {
      ...
      if (cur->is_leaf_) {
        //BT: FIXME, I disable this parameter computation for scalability
#ifndef CONCURRENCY
        stats_.num_node_lookups += cur->level_;
#endif
        auto leaf = static_cast<data_node_type*>(cur);
...
forceinline bool lock_parent_node(
  ...
  void get_depth_info() {
    ...
    uint64_t sum_keys = 0;
    uint64_t sum_keys_level_multi = 0;
    uint64_t sum_model_nodes = 0;
    uint64_t sum_model_size = 0;
    std::map<int, int> length_map;
    std::map<int, int> number_map;
    std::map<int, int> ratio_map;

    int total_meta_array_length = 0;
    int filled_array_length = 0;
    int total_overflow_items = 0;
    int total_scale_parameter_size = 0;
    int total_overflow_stash_blocks = 0;
    uint64_t DRAM_size = 0;
    uint64_t PM_size = 0;
    uint64_t all_slots = 0;
    uint64_t stash_capacity = 0;
    uint64_t primary_capacity = 0;
    uint64_t stash_insert = 0;
    uint64_t primary_insert = 0;
    ...
    for (NodeIterator node_it = NodeIterator(this); !node_it.is_end();
      ...
      if (cur->is_leaf_) {
        ...
        cur_data_node->num_keys_ = cur_data_node->total_keys_in_node();
        stash_insert += cur_data_node->stash_keys_in_node();
        //cur_data_node->validate_link_bitmap();
        //cur_data_node->validate_whether_num_counter_match();
        stash_capacity += cur_data_node->stash_capacity_;
        primary_capacity += cur_data_node->data_capacity_;
        
        auto stash_info = cur_data_node->collect_overflow_stash_blocks();
        total_scale_parameter_size += stash_info.first;
        total_overflow_stash_blocks += stash_info.second;
        all_slots += cur_data_node->get_static_slots();
        DRAM_size += cur_data_node->get_DRAM_size();
        PM_size += cur_data_node->get_PM_size();
        sum_keys += cur_data_node->num_keys_;
        sum_keys_level_multi += cur_data_node->num_keys_ * cur->level_;
        sum_nodes++;
        sum_level += cur->level_;
        if(cur->level_ > max_level) max_level = cur->level_;
        cur_data_node->collect_overflow_length_and_items(length_map, number_map, ratio_map, total_meta_array_length, filled_array_length, total_overflow_items);
      }else{
        sum_model_nodes++;
        sum_model_size += (reinterpret_cast<model_node_type*>(cur))->get_node_size();
      }
    }
    PM_size += sum_model_size;
    PM_size += (sizeof(*this) + sizeof(LogArray) + sizeof(RootExpandLog));

    std::cout << "The Max Node depth is " << max_level << std::endl;
    std::cout << "Sum model nodes = " << sum_model_nodes  << std::endl;
    std::cout << "Sum model node size = " << sum_model_size << std::endl;
    std::cout << "Sum model node size (MB) = " << (double)sum_model_size/(1024*1024) << std::endl;
    std::cout << "Sum level is " << sum_level << std::endl;
    std::cout << "Sum data nodes is " << sum_nodes << std::endl;
    std::cout << "The Everage Node depth per data node is " << (double)sum_level / sum_nodes << std::endl;
    std::cout << "The Everage Node depth per key is " << (double)sum_keys_level_multi / sum_keys << std::endl;
    std::cout << "The number of keys is " << sum_keys << std::endl;
    std::cout << "Overflow ratio = " << (double)stash_insert / sum_keys << std::endl;
    std::cout << "Stash raio = " << (double)stash_capacity /(primary_capacity + stash_capacity)  << std::endl;

    std::cout << "All total_meta_array_length = " << total_meta_array_length << std::endl;
    std::cout << "The meta_array which has overflows = " << filled_array_length << std::endl;
    std::cout << "The ratio that has overflows = " << (double)filled_array_length / total_meta_array_length << std::endl;

    std::cout << "Num keys in system = " << stats_.num_keys << std::endl;
    std::cout << "Total parameter size = " << total_scale_parameter_size << std::endl;
    std::cout << "Total overflow stash blocks = " << total_overflow_stash_blocks << std::endl;
    std::cout << "The ratio is " << total_overflow_stash_blocks / static_cast<double>(total_scale_parameter_size) << std::endl;
    
    std::cout << "The PM size (B) = " << PM_size << std::endl;
    std::cout << "The DRAM size (B) = " << DRAM_size << std::endl;
    printf("PM size (GB) = %.10f\n", PM_size / (1024.0 * 1024.0 * 1024.0));
    printf("DRAM size (GB) = %.10f\n", DRAM_size / (1024.0 * 1024.0 * 1024.0));
    std::cout << "PM / DRAM = " << PM_size / static_cast<double>(DRAM_size) << std::endl;
    std::cout << "The load factor = " << sum_keys / static_cast<double>(all_slots) << std::endl;

    /*
    auto iter = ratio_map.begin();
    for(; iter != ratio_map.end(); iter++){
      std::cout << "Ratio " << iter->first << " :" << iter->second << std::endl;
    }
    int total_linked_length = 0;
    iter = length_map.begin();
    for(; iter != length_map.end(); iter++){
      std::cout << "Length " << iter->first << " :" << iter->second << std::endl;
      total_linked_length += iter->first * iter->second;
    }

    std::cout << "----------------------------------------------------------------" << std::endl;
    std::cout << "Everage length in filled array is " << (double)total_linked_length / filled_array_length << std::endl;
    std::cout << "Everage number in filled array is " << (double)total_overflow_items / filled_array_length << std::endl;
    std::cout << "Everage number in all array is " << (double)total_overflow_items / total_meta_array_length << std::endl;
    iter = number_map.begin();
    for(; iter != number_map.end(); iter++){
      std::cout << "number " << iter->first << " :" << iter->second << std::endl;
    }
    */
  }
 ...
 public:
  ...
  void bulk_load(const V values[], int num_keys) {
    ...
    new (reinterpret_cast<model_node_type*>(tmp)) model_node_type(0, allocator_);
    root_node_ = reinterpret_cast<AlexNode<T,P>*>(tmp);
#else
    PMEMoid tmp;
    model_node_type::New(&tmp, 0);
 ...
 private:
  ...
  void create_superroot() {
    ...
    superroot_ = reinterpret_cast<model_node_type*>(tmp);
#else
    PMEMoid tmp;
    model_node_type::New(&tmp, 1);
  ...
  void bulk_load_node(const V values[], int num_keys, AlexNode<T, P>*& node,
    ...
    if (num_keys <= derived_params_.max_data_node_slots *
      ...
      int data_capacity = static_cast<int>(num_keys / 0.9);
      double overflow = data_node->compute_overflow_frac(values, num_keys, data_capacity, data_node_model);
      double stash_frac = std::max(0.05, std::min(0.3, overflow * 1.5));
      total_set_overflow_frac += stash_frac;
      ...
      total_overflow_frac += overflow;
      total_overflow_frac_with_weight += overflow * num_keys;
      int new_data_capacity = static_cast<int>(data_capacity * (1 - overflow));
      double new_overflow = data_node->compute_overflow_frac(values, num_keys, new_data_capacity, data_node_model);
      total_overflow_frac_after_adjust += new_overflow;
      total_overflow_frac_with_weight_after_adjust += new_overflow * num_keys;
      double predict_diff = new_overflow - overflow;

      if(predict_diff >= 0){
        positive_predict_overflow += predict_diff;
        positive_predict_overflow_with_weight += predict_diff * num_keys;
        positive_predict_overflow_keys += num_keys;
        positive_predict_overflow_times++;
      }else{
        negative_predict_overflow += predict_diff;
        negative_predict_overflow_with_weight += predict_diff * num_keys;
        negative_predict_overflow_keys += num_keys;
        negative_predict_overflow_times++;
      }

      if(new_overflow > max_predict_overflow_ratio) max_predict_overflow_ratio = new_overflow;
      if(new_overflow < min_predict_overflow_ratio) min_predict_overflow_ratio = new_overflow;
    ...
    if (best_fanout_tree_cost < node->cost_ ||
      ...
      auto model_node = reinterpret_cast<model_node_type*>(tmp);
      //model_node_num++;
      //model_node_size += model_node->get_node_size();
#else
      PMEMoid tmp;
      model_node_type::New(&tmp, fanout);
      ...
      for (fanout_tree::FTNode& tree_node : used_fanout_tree_nodes) {
        ...
        void *tmp;
        model_node_type::New(&tmp, 0);
        auto child_node = reinterpret_cast<model_node_type*>(tmp);
#else
        PMEMoid tmp;
        model_node_type::New(&tmp, 0);
  ...
  data_node_type* bulk_load_leaf_node_from_existing(
      ...
      bool keep_right = false, bool build_sorted_node = false, PMEMoid* reserved_ptr = nullptr, double stash_frac = 0.05) {
 ...
 public:
  ...
  void get_search_time_breakdown(){
    //std::cout << "Traversal levels = " << traversal_levels << std::endl;
    std::cout << "Traversal time = " << (double)model_traversal_ms / 1e9 << std::endl;
    std::cout << "Time in each traversal layer (ns) " << (double)model_traversal_ms/traversal_levels << std::endl;
    std::cout << "Data array lookup time = " << (double)data_lookup_ms / 1e9 << std::endl;
    std::cout << "Time in each data array lookup (ns) " << (double)data_lookup_ms/data_lookup_nums << std::endl;
    std::cout << "Overflow lookup time = " << (double)overflow_lookup_ms / 1e9 << std::endl;
    std::cout << "Time in each overflow traversal nums (ns) " << (double)overflow_lookup_ms/overflow_traversal_nums << std::endl;
    std::cout << "Time in each overflow traversal level nums (ns) " << (double)overflow_lookup_ms/overflow_traversal_level_nums << std::endl;
    std::cout << "Time in each meta-array traversal nums - dram stash + pm overflow (ns) " << (double)overflow_lookup_ms/meta_array_traversal_nums << std::endl;
    std::cout << "Overflow times = " << meta_array_traversal_nums << std::endl;
    std::cout << "overflow traversal levels/ Overflow times = " << (double)overflow_traversal_level_nums/meta_array_traversal_nums << std::endl;
    std::cout << "overflow traversal nums/Overflow times = " <<(double)overflow_traversal_nums / meta_array_traversal_nums << std::endl;
  }
...
RETRY:
 ...
 private:
  ...
  void expand_root(T key, bool expand_left) {
    std::cout << "I am expanding the root" << std::endl;
    ...
    if (expand_left) {
      ...
      auto key_difference = static_cast<double>(std::max(key, get_max_key()) -
                                                istats_.key_domain_max_);
      std::cout << "finish Get the key difference" << std::endl;
    ...
    if (root->num_children_ * expansion_factor <= derived_params_.max_fanout) {
      ...
      auto new_root_node = reinterpret_cast<model_node_type*>(tmp);
      ...
      auto new_root = reinterpret_cast<model_node_type*>(tmp);
#else
      PMEMoid tmp;
      model_node_type::New(&tmp, expansion_factor);
    ...
    if (expand_left) {
      ...
      std::cout << "I am calculate the left boundary" << std::endl;
      int left_boundary = outermost_node->lower_bound(left_boundary_value, true, true);
      std::cout << "Finish calculating the left boundary" << std::endl;
      ...
      for (int i = new_nodes_end; i > new_nodes_start; i -= n) {
        ...
        std::cout << "start the bulk loading left" << std::endl;
      ...
      std::cout << "I am calculating the right boundary" << std::endl;
      int right_boundary = outermost_node->lower_bound(right_boundary_value, true, true);
      std::cout << "Finish calculating the right boundary" << std::endl;
      ...
      for (int i = new_nodes_start; i < new_nodes_end; i += n) {
        ...
        std::cout << "Start the bulk loading right" << std::endl;
        data_node_type* new_node = bulk_load_leaf_node_from_existing(
            outermost_node, left_boundary, right_boundary, true, nullptr, false, false,false);
        std::cout << "End the bulk loading right" << std::endl;
        new_node->level_ = static_cast<short>(root->level_ + 1);
        new_node->local_depth_ = new_local_depth;
        if (prev) {
    ...
    std::cout << "I finish expand the root" << std::endl;
  }
...
RETRAVEL:
    ...
    { 
      ...
      void *tmp;
      model_node_type::New(&tmp, fanout);
      auto new_node = reinterpret_cast<model_node_type*>(tmp);
#else
...
void create_two_new_data_nodes_without_parent(data_node_type* old_node, bool reuse_model, PMEMoid *split_left, PMEMoid *split_right, bool build_sorted_node = false, double stash_frac = 0.05) {
    ...
    data_node_type* left_leaf = bulk_load_leaf_node_from_existing(
        ...
        append_mostly_left && appending_left_fit , build_sorted_node, split_left, stash_frac);
    ...
    data_node_type* right_leaf = bulk_load_leaf_node_from_existing(
        ...
        reuse_model,
        append_mostly_right && appending_right_fit,
        append_mostly_left && appending_left_fit, build_sorted_node, split_right, stash_frac);
    right_leaf->min_limit_ = left_leaf->max_limit_;
...
void create_new_data_nodes_without_parent(
 ...
 public:
  ...
  void validate_key_range_recursive(const AlexNode<T, P>* node){
    if (!node) return;
    if (node->is_leaf_) {
      // do nothing
    } else {
      // validate all the child nodes of the model node
      int cur = 0;
      while (cur < node->num_children_) {
        AlexNode<T, P>* child_node = node->children_[cur];
        // validate whether the range of this children is valid
        //int repeats = 1 << child_node->duplication_factor_;
        int repeats = 1 << (log_2_round_down(node->num_children_) - child_node->local_depth_); 
        auto model_parent = reinterpret_cast<model_node_type*>(node);
        if(child_node->is_leaf_){
          // validate whether the min key and max key are valid
          double test_lleft = (cur - model_parent->model_.b_) / model_parent->model_.a_;
          double test_rright = (cur + repeats - model_parent->model_.b_) / model_parent->model_.a_;

          auto data_node = reinterpret_cast<data_node_type*>(child_node);
          if(data_node->min_key_ < test_lleft) std::cout << "Smaller than the left" << std::endl;
          if(data_node->max_key_ > test_rright) std::cout << "Larger than the right" << std::endl;
        }else{
          // validate whether the model range is valid


        }
        validate_key_range_recursive(child_node);
        cur += repeats;
      }
    }
  }
