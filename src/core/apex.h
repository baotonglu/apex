      ...
      if (cur == child_node) {
        ...
        if (cur_node->children_[ID] == child_node) {
          ...
          if (write_lock) {
            ...
            if (cur_node->get_read_lock()) {  // it could be locked so that it
                                              // is not deleted
              return true;
            }
          }
    ...
    if (fail) {
      ...
      if (fail == 5)  // Data node resizing
      {
        ...
        my_alloc::BasePMPool::Persist(
            parent->children_ + start_bucketID,
            sizeof(node) * (end_bucketID - start_bucketID));
 ...
 private:
  ...
  RETRAVEL:
    ...
    if (should_split_downwards) {
      ...
      my_alloc::BasePMPool::Persist(
          parent->children_ + start_bucketID,
          sizeof(new_node) * (end_bucketID - start_bucketID));
      ...
      if (fanout > repeats) {
        ...
        my_alloc::BasePMPool::Persist(
            grand_parent->children_ + parent_start_bucketID,
